...
class ArucoDetector:
    def __init__(self):
        ...
        self.offset_distance = 0.1  # 在标记前方10cm
                ...
                if success:
                    ...
                    if self.marker_pose_camera is not None:
                        ...
                        if self.target_pose_camera is not None:
                            ...
                            pose_stamped.pose = self.marker_pose_camera
    ...
    def get_target_pose(self):
        """获取目标位姿"""
        return self.target_pose_camera, self.marker_detected
...
class MoveItController:
    ...
    def plan_to_marker(self):
        """规划机械臂移动到标记位置"""
        # 获取目标位姿
        target_pose_camera, marker_detected = self.aruco_detector.get_target_pose()
        
        if not marker_detected or target_pose_camera is None:
            rospy.logwarn("未检测到标记或目标位姿无效")
            return False
        
        try:
            # 转换位姿到基坐标系
            pose_in_base = self.transform_pose_to_base(target_pose_camera)
            ...
            self.move_group.set_pose_target(pose_in_base.pose, self.tool_frame)
