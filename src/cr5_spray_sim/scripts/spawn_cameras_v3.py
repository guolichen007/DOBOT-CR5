#!/usr/bin/env python3
"""
V3 Camera Spawner: spherical-coordinate cameras with pedestals.
Computes camera positions from (radius, azimuth, elevation) relative to object.
"""
import os
import sys
import math
    ...
    if not config_path:
        try:
            ...
            config_path = os.path.join(
                ...
                "config", "scene_v3.yaml")
        except Exception:
            config_path = os.path.join(
                os.path.dirname(__file__), "..", "config", "scene_v3.yaml")
    with open(config_path) as f:
        return yaml.safe_load(f)


def spherical_to_cartesian(radius, azimuth_deg, elevation_deg, target):
    """
    Convert spherical coords (relative to target) to world position.
    azimuth_deg: 0=+X (rear), 90=+Y (left), 180=-X (front), 270=-Y (right)
    elevation_deg: upward from horizontal
    """
    az = math.radians(azimuth_deg)
    el = math.radians(elevation_deg)

    # Direction from camera to target in world frame
...
def compute_look_at(cam_pos, target_pos, roll_offset_deg=0.0):
    ...
    cmd = [sys.executable, script,
           ...
           "--roll-offset-deg", str(roll_offset_deg), "--yaml"]
...
class CameraSpawnerV3:
    def __init__(self):
        rospy.init_node("spawn_cameras_v3")
        self.scene = load_scene_config()

        profile_name = rospy.get_param("~camera_profile", "vm")
        rig_cfg = self.scene.get("camera_rig", {})
        profiles = rig_cfg
        ...
        cameras = rig_cfg.get("cameras", [])
        if not cameras:
            rospy.logerr("No cameras in scene_v3.yaml!")
            return

        # Target: workpiece position
        wp = self.scene.get("workpiece", {})
        wp_pos = wp.get("position", {"x": 0.60, "y": 0.0, "z": 0.94})
        self.target = [wp_pos["x"], wp_pos["y"], wp_pos["z"]]

        # Optional camera_offset: if workpiece is not at object_frame, adjust target
        camera_offset = self.scene.get("camera_rig", {}).get("target_offset", {})
        if camera_offset:
            self.target[0] += camera_offset.get("x", 0)
            self.target[1] += camera_offset.get("y", 0)
            self.target[2] += camera_offset.get("z", 0)

        rospy.loginfo("Camera target: (%.3f, %.3f, %.3f)", *self.target)
        ...
        for cam in cameras:
            name = cam["name"]
            radius = cam.get("radius", rig_cfg.get("radius", 0.98))
            az = cam.get("azimuth_deg", 0)
            el = cam.get("elevation_deg", 15)
            roll_off = cam.get("roll_offset_deg", 0.0)

            # Compute world position from spherical coords
            pos = spherical_to_cartesian(radius, az, el, self.target)
            rospy.loginfo("%s: sphere(r=%.2f, az=%.0f°, el=%.0f°) -> world(%.3f, %.3f, %.3f)",
                          name, radius, az, el, pos[0], pos[1], pos[2])
            self.spawn_one(name, pos, self.target, roll_offset_deg=roll_off)
    ...
    def spawn_one(self, name, pos, tgt, roll_offset_deg=0.0):
        ...
        try:
            ...
            if resp.success:
                dist = rpy_data["distance_m"]
                rospy.loginfo(
                    "Spawned %s: dist=%.2fm look-err=%.4f° up-err=%.1f° "
                    "rpy=(%.3f,%.3f,%.3f)",
                    name, dist, err, up_err, roll, pitch, yaw)
            else:
