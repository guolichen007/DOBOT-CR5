...
#
# 用法:
#   bash run_scene_v337.sh --gui --object=motor_housing_cylinder
#   bash run_scene_v337.sh --isolated --object=motor_housing_cylinder  # headless
# ============================================================================
set -euo pipefail
...
用法:
  bash run_scene_v337.sh [OPTIONS]
...
示例:
  ...
  bash run_scene_v337.sh --gui --object=motor_housing_cylinder
  # Headless 自动测试 (随机端口)
  bash run_scene_v337.sh --isolated --strict
EOF
    exit 0
}
...
LOG_DIR="/tmp/cr5_spray_v337_${SESSION_ID}"
