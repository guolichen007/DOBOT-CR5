...
def load_yaml_config():
    """从 config/calibration_target_v1.yaml 加载面板 pose."""
    # 尝试多种路径: rospack, 相对路径
    config_path = None
    candidates = []
    ...
    try:
        ...
        candidates.append(os.path.join(pkg_path, "config", "calibration_target_v1.yaml"))
    except Exception:
        pass

    # 相对于脚本路径
    script_dir = os.path.dirname(os.path.abspath(__file__))
    candidates.append(os.path.join(script_dir, "..", "config", "calibration_target_v1.yaml"))

    # 工作目录
    candidates.append("src/cr5_spray_sim/config/calibration_target_v1.yaml")
    ...
    if config_path is None:
        rospy.logerr("Cannot find calibration_target_v1.yaml, tried: %s", candidates)
        raise FileNotFoundError("calibration_target_v1.yaml not found")
